...
const weekAdjust = d => {
  ...
  const yearOffset = dateFns.getYear(d) - 2019;
  // TODO see if yearOffset is fine to use w/ 0 and 53 shifts
  if (month === 11 && isoWeek === 1) {
    return 53 + yearOffset * 52;
  } else if (month === 0 && isoWeek >= 52) {
    return 0 + yearOffset * 52;
  } else {
    return isoWeek + yearOffset * 52;
  }
};
const getCalAggs = d => {
